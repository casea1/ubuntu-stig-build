...
**ROOT CAUSE, established 2026-09-08 on dev-16 by asking apt to install the FIPS libraries ALONE, with no metapackage in the way:**
```
apt-get -s install libgnutls30t64=3.8.3-1.1ubuntu3.6+Fips1.2 \
                   libgcrypt20=1.12.0-2ubuntu0.1~Fips1~rc11
  libgcrypt20 : Depends: libgpg-error0 (>= 1.56)
                but 1.47-3build2.1 is to be installed
```
The `fips-updates` noble suite publishes `libgcrypt20 1.12.0-2ubuntu0.1~Fips1~rc11` -- a **major upstream bump** from noble's own 1.10.3 -- which needs `libgpg-error0 >= 1.56`. Noble ships **1.47**. **NOT YET VERIFIED whether the suite publishes a newer libgpg-error0** -- only that apt had no newer candidate available in that transaction. If it does publish one, suspect `libgpg-error0:i386` holding the amd64 copy down by `Multi-Arch: same`, exactly as with libgcrypt20 -- which would make this ours after all. `apt-cache policy libgpg-error0` in the retry window settles it. So the FIPS libgcrypt cannot be installed, `ubuntu-fips-userspace`'s dependency on it cannot be met, and the enable fails. Nothing on the box fixes this. It is a Canonical publication gap: a component was rebuilt from a newer upstream without its own dependency being shipped alongside.
**This also explains why boxes built weeks earlier have FIPS and new ones cannot.** Canonical's FIPS tracker lists noble at `~Fips1~rc7`, which is 1.10.3-based and satisfied by noble's libgpg-error0. The suite has since moved to the 1.12.0-based `~rc11`. Boxes that enabled FIPS before that republish are fine and stay fine; boxes enabling after it cannot. Not a fleet difference, a calendar one.
**Two earlier readings here were wrong, and both mistakes are instructive.**
1. *"`libgnutls30` does not exist on noble."* No -- test A proves the FIPS gnutls carries `Provides: libgnutls30 (= 3.8.3-1.1ubuntu3.6+Fips1.2)`. That half was always satisfiable and only ever failed as a cascade. An empty `apt-cache policy` version table means **virtual**, not absent.
2. *"i386 multiarch is the cause."* No -- the blocker is `libgpg-error0` on amd64, which i386 cannot touch. But the i386 conflict is **real and next in line**: `libgcrypt20` is `Multi-Arch: same` and the suite is amd64-only, so the moment Canonical ships `libgpg-error0 >= 1.56` this box would fail again on the i386 sibling. `pro_attach` running before `fpga_tools` is therefore still correct, just not what is stopping FIPS today.
**The technique that finally worked, after two wrong answers from reading pro's output and one from reading apt's:** take the metapackage out of the question and ask apt to install the leaf packages at exact versions. A metapackage reports which of ITS dependencies is unmet; only the leaf reports WHY. Do not truncate `apt-cache showpkg` with `head` either -- `Reverse Provides:` is at the end, and cutting it is what made the previous test unreadable.
...
| `it-serial` *(development)* | USB serial adapters the kernel does not recognise. `status` (default — plugged in, bound, and whether a normal user can open the port), `bind`, `add <vid:pid>` (bind now **and** persist to `site.yml`), `ports`. `ftdi_sio` only binds IDs in its compiled-in table, so a Sealevel adapter (`0c52:e402`) enumerates and produces no `/dev/ttyUSB*` at all, with nothing logged — it reads as dead hardware. The ID is added to the driver's table by a boot-time oneshot, because that table lives in the module and is lost on every reboot |
