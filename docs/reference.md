...
**RE-EXAMINED 2026-09-08. The evidence points at ONE cause, and it is ours: i386 multiarch.** An earlier reading here said `ubuntu-fips-userspace` depends on a package noble does not have. That was wrong, and the mistake is worth keeping because it is easy to repeat.
`apt-cache policy libgnutls30` printed a stanza with an EMPTY version table, and I read that as "no such package". It is not: that is exactly what apt prints for a **virtual package** -- a name that exists only as a `Provides`. A genuinely unknown name gets `N: Unable to locate package` instead, which is what the same box printed for `usg` when its repo was absent. **Two different states, two different outputs, and I read the wrong one.**
So `libgnutls30` is almost certainly provided by `libgnutls30t64`, and the dependency is unmet not because the name is missing but because **the provider at that version cannot be installed** -- for the same reason `libgcrypt20` cannot: `Multi-Arch: same` forces the amd64 and i386 copies to one version, the `fips-updates` suite is **amd64-only**, and `fpga_tools` puts BOTH `libgcrypt20:i386` and `libgnutls30t64:i386` on a development box (dev-16 showed both installed). Neither can move, so neither dependency resolves. One cause, two symptoms, and the asymmetry in apt's output is only because `libgcrypt20` is a real package name (so apt can name the version it is keeping) while `libgnutls30` is virtual (so it cannot).
That means **FIPS is not broken upstream** and these boxes do not need reimaging -- but it also means **FIPS and the FPGA i386 stack are probably mutually exclusive on one box**, since with FIPS installed on amd64 no i386 build of those libraries exists at the matching version, and `libsystemd0:i386` (which most of the i386 GUI set needs) depends on `libgcrypt20`. That is an architecture decision, not a bug.
Confirm both halves in one shot, inside pro's retry window, without changing anything:
```
sudo sh -c 'pro enable fips-updates --assume-yes >/tmp/f.log 2>&1 &
  sleep 15
  apt-cache showpkg libgnutls30 | head -15          # "Reverse Provides:" = virtual
  apt-get -s install ubuntu-fips libgcrypt20:i386- libgnutls30t64:i386-
  wait'
```
The `pkg:i386-` suffix asks apt to simulate REMOVING those siblings while installing `ubuntu-fips`. If that resolves, i386 was the whole story.
**The general lesson: an empty `apt-cache policy` version table means VIRTUAL, not absent, and a dependency line with no "but ... is to be installed" clause means the depended-on name has no installable provider -- which can be a cascade from a different package entirely, not a missing one.**
