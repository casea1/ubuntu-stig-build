...
Every file `ai_compose` writes has an explicit `mode:`, which is the second case — so a default ACL on the directory is removed from each file on **every pull**. The entries therefore have to be applied to the FILES as well, after they are written, which is why `ai_ops_access.yml` runs last in the role. `it-aiops` sets no default ACL at all: under one, a file an admin creates by hand ignores the STIG's `umask 077`.

**An explicit entry, unlike an inherited one, WIDENS a `0600` file** — `setfacl -m g:aiops:rw` recalculates the mask and grants it. The site owner has accepted read + write for the `aiops` managers on everything under `/opt/stacks` and `/opt/docker`, every `.env` included (read 2026-09-25, write 2026-09-26). `it-aiops` also sets a default entry on each folder, so files and stacks created later are editable at once; the owning `sudo` group is not widened, because its own `group::` entry is left alone and the mask only limits. **Private keys are the exception, and not by policy:** once a file carries an ACL entry its mode reads `0640`/`0660` (the group bits are the mask), and OpenSSH refuses a key others can read — `WARNING: UNPROTECTED PRIVATE KEY FILE! Permissions 0640 ... are too open`, verified on noble's 9.6p1. magpie bind-mounts `./ssh` live, so granting its key would break git sync at the next scheduled run with no container restarted. `it-aiops` finds keys by content, holds them back, and sets no default entry on an `ssh` folder so a key created there later is not caught either.

`off` removes the default ACLs too, not just the named entries: a default ACL left with only its base entries still overrides the umask for every new file.

Tested with real unprivileged members: every `.env` and compose file editable; one member's new `compose.override.yaml` and a new stack folder editable by another; a file root made under `umask 077` editable; the private key denied and both it and a key created afterwards still accepted by `ssh-keygen -y`; a `sudo`-group admin still read-only on `compose.yaml`; after an ansible-style rewrite of `.env` (new inode, `0600`) the role's final `it-aiops` step restores the grant; after `off`, a new file is `0600` again.

**Do not solve this with the `docker` group.** Membership is root-equivalent — `docker run -v /:/host` is a root shell on the host — so on a STIG box it is a privilege escalation and a finding, and it is not logged. The sudo grant (`it-aiops`) is command-scoped and goes through sudo's audit trail instead — and it is **exact argument forms, not bare commands**. The first version named `it-docker`, `it-stack-diff` and `it-baseline` bare and `NOPASSWD`; a bare command permits any arguments (trap 12ad), and `it-stack-diff --out FILE` writes with `tee` as root, so it would have let a member of a *review* group empty `/etc/shadow` without a password. It also granted `it-docker restart|stop|start`. Caught before it was ever enabled; tested with a real member: the old forms are refused by sudo, the look-only forms run, and `it-stack-diff` refuses `--out` for a non-admin on its own. Members type `sudo` in front — the scripts are in `/opt/it`, which they cannot traverse. Procedure: [procedures.md §5.11](procedures.md#511-give-the-ai-stack-managers-access-aiops).
...
| `it-aiops` | AI stack managers (`aiops`): `on`/`off`/`refresh`/`add`/`remove` read + write on everything in `/opt/stacks` + `/opt/docker` except private keys, by hand, no pull; bare = status, opened as a member |
