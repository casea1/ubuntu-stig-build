...
**41. A role that writes into `/opt/it/scripts` must create it, because `it_scripts` runs far too late to help.** `dev_tools` ships `it-vscode` and `it-codeserver` into that directory at line 164 of `local.yml`; the role that creates it is at line 279. On every box that had ever pulled before, the directory was already there and nothing was wrong -- so this survived the whole fleet and only surfaced on **dev-16's first build**: `Destination directory /opt/it/scripts does not exist`, and with it the usual consequence of any failed task, that every role after it (`fpga_tools`, `remote_desktop`, `usg_harden`, `usg_remediate`, `usbguard`, `desktop_branding`, `powerstrux`, `it_scripts`) never ran and the box came up **unhardened**. `copy` does not create a missing parent. This was the third time -- `fpga_tools`, `usb_serial` and `remote_desktop` were each fixed alone, one box at a time -- so it is now checked instead of remembered: `tools/check-managed-dirs.py` parses the task YAML (regex could not follow the loops of inline dicts or the Jinja paths) and fails on any role writing into a shared directory it does not create. Two lessons worth separating: a first build exercises ordering that no subsequent pull ever will, and "it works because of where the role sits in `local.yml`" is not a property anything enforces.
