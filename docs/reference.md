...
**CONFIRMED 2026-09-08 on dev-17, and it is TWO blockers, not one.** `pro` deletes the repo on failure, so the only way to ask apt anything is during its ~20s retry window: start the enable in the background, `sleep 15`, then query. That gives the real picture.
**(a) `ubuntu-fips-userspace` has a broken dependency and this is NOT fixable on the box.** `1.4.0~rc13+updates4` requires `libgnutls30 (= 3.8.3-1.1ubuntu3.6+Fips1.2)`. `apt-cache policy libgnutls30` -> **`Candidate: (none)`** -- no such package on noble, the time_t transition renamed it. And **the FIPS suite publishes the right one**: `libgnutls30t64` shows `Candidate: 3.8.3-1.1ubuntu3.6+Fips1.2` from `esm.ubuntu.com/fips-updates` at pin 1001. The metapackage is asking for a name its own repository does not carry. That is a Canonical packaging defect; open a case. Note this corrects an earlier reading here that the suite was missing the package -- it is not, the dependency is simply written against the pre-t64 name.
**(b) `libgcrypt20` is held down by i386 multiarch, which IS ours.** Its FIPS version is the candidate at pin 1001 and apt still reports `but 1.10.3-2ubuntu0.2 is to be installed`: `Multi-Arch: same` forces amd64 and i386 to one version, the FIPS suite is amd64-only, and `fpga_tools` puts `libgcrypt20:i386` on the box via `libsystemd0:i386`. Enabling FIPS before i386 exists is the fix (`pro_attach` runs ahead of `fpga_tools`) -- **necessary but not sufficient while (a) stands.**
Both FIPS versions are release candidates (`~rc13`, `~rc11`) and `pro status --all` shows plain `fips` and `fips-preview` as `n/a` for noble. Treat noble FIPS as not yet shipping. **The lesson for the next one: a dependency line with no "but ... is to be installed" clause means NO CANDIDATE, and a line with one means a candidate exists that something else is blocking. Two different failures that look identical in pro's output, and only `apt-cache policy` inside the retry window tells them apart.**
