...
- **Docling VLM (System 2).** `docling-serve` runs **granite-docling-258M** locally on the System-2 GPU — docling's default `granite_docling` VLM preset, a tiny 258M vision-language model that converts a page image straight to DocTags. **This image runs with `--artifacts-path` (runtime model downloads DISABLED)** and resolves models from `/opt/app-root/src/.cache/docling/models` in **docling's own layout** (`ibm-granite--granite-docling-258M`), *not* an HF cache — so the model must be **added with docling's own tool**, `docling-tools models download-hf-repo`. We persist that dir as the **`docling-models`** volume (mounting the empty volume seeds it with the image's baked-in models), and `ai_compose` adds granite-docling into it (opt-in, `ai_model_fetch`). Because downloads are disabled, it's inherently **air-gap-safe** once the volume is populated — `it-model-export`/`it-model-import` carry the whole populated artifacts dir on USB. The VLM pipeline is selected **per conversion request**; docling's standard layout/OCR pipeline stays the default for a plain convert. Add it by hand on a running box:
  ```bash
  sudo docker exec docling-serve docling-tools models download-hf-repo ibm-granite/granite-docling-258M
  ```
  A missing model fails a VLM convert with `Model 'ibm-granite/granite-docling-258M' not found in artifacts_path`.
