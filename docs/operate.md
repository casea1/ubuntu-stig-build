...
- **Dockge (`ai_stack`, `dockge_enabled`):** a Docker-Compose-stack management web UI on `http://‹host›:9001` (opened by `ai_firewall`, rate-limited; HTTP, so use `http://`). **Create the admin account on first login.** It mounts the Docker socket (root-equivalent), so restrict its port to admins (add a `from:` entry in `ai_firewall_allow_ports`) or front it with a proxy. Dockge manages compose stacks under **`dockge_stacks_dir`** (`/opt/stacks`), bind-mounted at the same path in the container. To manage the baked AI stack from Dockge, symlink it in: `sudo ln -s /opt/it/docker /opt/stacks/ai` (Dockge expects a `docker-compose.yaml`/`compose.yaml` per stack folder). Update it later with `docker pull <dockge_image> && docker rm -f dockge` then re-run the play (`dockge_image` in group_vars). Set `dockge_enabled: false` to skip. (Dockge replaced Portainer; the build removes an old `portainer` container and its `9443` firewall rule on re-run.)
