...
**An explicit entry, unlike an inherited one, WIDENS a `0600` file.** `setfacl -m g:aiops:r` recalculates the mask and grants the read. So the first version — every non-hidden file except `.env` — would have handed the group magpie's git SSH keys in `/opt/stacks/magpie/ssh` (`root:root 0700`). Caught before it was ever enabled. `it-aiops` grants by rule instead of by exception: **what an admin sees without sudo** — other-readable, or group-readable with the owning group `sudo` — and nothing inside a folder that fails the test. It reads the owning group's bits from the ACL's `group::` entry, not the mode, because once an entry exists the mode's group bits are the mask and a granted root-only file would pass every later run. It also strips any entry that fails the rule, so the first version's leftovers go too.

Tested with real unprivileged accounts: `compose.yaml`, `nginx.conf`, `magpie_config.yaml`, build contexts readable; every `.env`, the SSH key, `.oikb.yaml`, a `root:sudo 0600` file and a symlink to `/etc/shadow` denied.
...
| `it-aiops` | AI review team (`aiops`): `on`/`off`/`refresh`/`add`/`remove` read access to `/opt/stacks` + `/opt/docker` by hand, no pull; bare = status + leak check |
