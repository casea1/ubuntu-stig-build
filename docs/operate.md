...
- **Docling VLM (System 2).** `docling-serve` runs **granite-docling-258M** locally on the System-2 GPU — docling's default `granite_docling` VLM preset, a tiny 258M vision-language model that converts a page image straight to DocTags. No separate vLLM server: docling loads it via transformers, and the weights persist in the **`docling-models`** volume (mounted as docling's `HF_HOME=/opt/hf-cache`), so they download **once** and survive container recreates. **In the lab (online)** docling fetches the model on first VLM conversion; **air-gapped**, pre-stage it with `it-model-export`/`it-model-import` (it travels in HF-cache format) and set `ai_docling_hf_offline: 1` (→ `DOCLING_HF_OFFLINE=1`, `HF_HUB_OFFLINE`) so docling never reaches out to huggingface.co. The VLM pipeline is selected **per conversion request** (`granite_docling` is the default preset); docling's standard layout/OCR pipeline stays the default for a plain convert unless the client asks for the VLM pipeline. Verify it loaded: `sudo docker logs docling-serve | grep -i granite` after a VLM conversion.
