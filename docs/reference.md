...
**Half of it is self-inflicted, and it is an ORDERING bug.** `libgcrypt20` is `Multi-Arch: same`, so its amd64 and i386 copies must hold the identical version, and the `fips-updates` suite publishes **binary-amd64 only**. `fpga_tools` (development profile) turns on i386 multiarch, and `libgcrypt20:i386` arrives via `libsystemd0:i386` -- confirmed on dev-16 with `apt-cache rdepends --installed`. From that moment apt cannot move the amd64 copy to the FIPS version without an i386 build that does not exist, and the enable is unsatisfiable on a box that would have taken it an hour earlier. `fpga_tools` ran at local.yml:179 and the FIPS enable at :218, so the baseline was breaking its own FIPS enable. The attach and the FIPS enable now live in **`pro_attach`**, which runs before `dev_tools`/`fpga_tools`; `usg_harden` includes the same role (guarded on `_usg_active is not defined`) so the logic is not duplicated and the second pass does not re-run a failing enable. **Still unsolved:** the other unmet dependency, `libgnutls30 (= 3.8.3-1.1ubuntu3.6+Fips1.2)`, against a box carrying `libgnutls30t64` after noble's time_t transition. Ordering does not touch that one, so treat FIPS on noble as unproven until a box actually reports `fips-updates enabled`.
