...
| 4 | Anti-virus | **Met (with deviation)** | ClamAV on all profiles (daemon + weekly scan). Docker volumes excluded — scanning 60 GB of model weights is pointless I/O. **FIPS breaks ClamAV silently** — OpenSSL in FIPS mode will not initialise MD5, so the engine loads its signatures and scans 0 bytes while reporting every file clean (confirmed on ASP-2, 2026-08-26). The `clamav_fips` role carves MD5 back in for the ClamAV processes only (**deviation — POA&M**); the kernel stays FIPS. `it-clamav test` is the check that settles it — until it passes on a given box, treat that box as having no antivirus. Signatures also go stale air-gapped; **`it-clamav install` is the manual path** — drop a signature `tar.gz` in `/opt/it/clamavsigs`, it validates the CVD digital signature before installing and confirms with an EICAR test. | `sudo it-clamav test` |
