...
| `it-usb` | `usb-guard.sh` | USBGuard device allow-list. `list` renders the devices as a **tree** — decoded USB class (hub / HID / MASS STORAGE / network), port, and each device nested under what it is plugged into — with `!` marking the classes that can act on their own (keyboards, storage, radios). `--raw` gives usbguard's own output. `enroll` is the guided whitelist; `allow`/`block` take either the device number or the `vendor:product` id, with `--all` for a pair matching several devices. `allow --permanent` falls back to appending the device's own full descriptor when usbguard refuses the upsert — which it does whenever more than one existing rule already matches, the usual case with two identical hubs. **`trust <vid:pid> [serial]`** pre-authorises by id *without the device present* — needed for hardware that disconnects itself when the host does not authorise it in time (write blockers do this). **A serial-scoped rule only covers that one unit** — a replacement of the same model has a different serial, will not match, and (being a write blocker) gives up and disconnects after ~6 seconds, so `it-usb blocked` shows nothing by the time you look. Trust the id without a serial to cover every unit of the model, and read the dmesg line: an authorised device is followed by `authorized to connect`, a blocked one is not. Also `status`/`blocked`/`policy`/`regenerate`. |
...
| `it-vulnscan` | `vuln-scan.sh` | **EMI profiles only.** The Linux counterpart to the org's `MUSA_Vuln_Scan` Windows job: `nmap -sV --script vuln` against this host, then a full anti-virus scan, both appended to one dated report in `/opt/ia/vulnscans`. Gates the AV half on an EICAR self-test, so a scanner that cannot detect is recorded as `ENGINE-FAULT` rather than `CLEAN`. `--quick` skips the AV scan; `--list`/`--show` read past reports; `--target` exists for a deliberate, authorised scope. Exits non-zero when a vuln script flagged something or the AV scan found/faulted. |
...
```bash
sudo it-vulnscan                 # nmap vuln scan + full AV scan
sudo it-vulnscan --quick         # nmap only (minutes, not hours)
sudo it-vulnscan --list          # past scans, newest first
sudo it-vulnscan --show          # read the newest one
```

Reports land in **`/opt/ia/vulnscans/<host>-vuln-scan-MM-DD-YYYY.txt`**, appended, so two runs on the same day are both in the record rather than one silently replacing the other. Retention is 12 sets (`--keep`).

**EMI profiles only.** `nmap` is installed by `base_packages/emi.yml`, and `it_scripts` places the command only where `it_scripts_emi_enabled` — actively removing it elsewhere, so a box rebuilt under another profile does not keep a command pointing at an nmap that is no longer there.

**It scans this host, on loopback.** That is what makes it evidence about the box's own listening services rather than a network sweep. `--target` exists for a deliberate, authorised scope; scanning anything you have not been authorised to scan is a different activity with different rules.

**The AV half is gated on an EICAR self-test.** On a FIPS box the host engine loads every signature and then detects nothing (see the ClamAV section below), so a "clean" full scan would be worthless as evidence. If the self-test fails the report records `ENGINE-FAULT`, not `CLEAN`, and the command exits non-zero. Engine selection is the same everywhere in this repo: containerised clamd first, then the host daemon, then standalone `clamscan`.

The summary line at the end is what an operator actually needs — open port count, how many lines a `vuln` script flagged, and the AV verdict — because nmap's own output buries all three. Exit is non-zero when a vuln script flagged something or the AV scan found/faulted, so it can gate a scheduled job. `it-checklist` item 28 reads the newest report and FAILs on findings, on a scan older than 45 days, or on never having run.
...
**The three databases age very differently.** `daily` is published several times a day and is the only one whose age is a finding; `main` and `bytecode` are published a couple of times a *year*, so a build date months back is normal and not a problem. If newer ones existed, freshclam would already have taken them — which is also the quickest way to answer "should I update main.cvd?": if `daily` is current, freshclam ran, and freshclam checks all three every time.

**Keeping the scanner in step with freshclam.** While a box still has a network, freshclam updates `/var/lib/clamav` — the *host* database. The containerised engine reads its own copy, so without help the host looks current while the thing that actually scans falls behind. `clamav-container-sync.path` watches the host directory and copies new databases across, reloading the scanner. `it-clamav check` compares the two versions and says so if they drift; `it-clamav sync` does it by hand.
...
#### Getting back off the container

The container is a workaround for [clamav#1786](https://github.com/Cisco-Talos/clamav/issues/1786), not an end state. When a fixed ClamAV lands, scanning should go back to the host engine.
...
sudo it-clamav revert            # hands scanning back, verifies, keeps the image
sudo it-clamav revert --purge    # ...and deletes the image + container database
```

`revert` **refuses to proceed unless the host `clamscan` detects an EICAR file first**, so it cannot leave a box with nothing working. It then copies the container's database back — the container has been the one receiving updates, so that database is the current one and `/var/lib/clamav` may still hold the imaging-day set — removes the units, unmasks the daemon, and runs a final EICAR test through the host daemon before reporting success.

Without `--purge` the image and `/var/lib/clamav-container` are left on disk, so putting the container back is just another `ansible-pull`.
