...
| `it-usb` | `usb-guard.sh` | USBGuard device allow-list. `list` renders the devices as a **tree** — decoded USB class (hub / HID / MASS STORAGE / network), port, and each device nested under what it is plugged into — with `!` marking the classes that can act on their own (keyboards, storage, radios). `--raw` gives usbguard's own output. `enroll` is the guided whitelist; `allow`/`block` take either the device number or the `vendor:product` id, with `--all` for a pair matching several devices. `allow --permanent` falls back to appending the device's own full descriptor when usbguard refuses the upsert — which it does whenever more than one existing rule already matches, the usual case with two identical hubs. **`trust <vid:pid> [serial]`** pre-authorises by id *without the device present* — needed for hardware that disconnects itself when the host does not authorise it in time (write blockers do this). Also `status`/`blocked`/`policy`/`regenerate`. |
