...
> **FIPS mode is currently NOT enabled on the 24.04 development boxes, and as of 2026-09-08 the evidence points at a configuration conflict of ours rather than an upstream defect.** `usg_enable_fips` is still `true` and `pro_attach` still attempts the enable; it fails, the box is warned, and the rest of the baseline applies.
>
> The conflict: the `fips-updates` suite is published **amd64-only**, `libgcrypt20` and `libgnutls30t64` are `Multi-Arch: same` (their amd64 and i386 copies must hold one version), and `fpga_tools` enables i386 multiarch on the development profile and pulls i386 copies of both in. Neither can move to the FIPS version, so `ubuntu-fips-userspace` finds neither dependency satisfiable. `pro_attach` now runs the enable **before** `fpga_tools`, so a box built from scratch on the current baseline should take FIPS.
>
> **The consequence to plan around: FIPS and the FPGA i386 stack are probably mutually exclusive on the same machine.** With FIPS installed on amd64 there is no i386 build at the matching version, and `libsystemd0:i386` -- which most of the i386 GUI set needs -- depends on `libgcrypt20`. So an engineering workstation can be FIPS **or** run the 32-bit halves of Vivado/Libero, not both. That is an architecture decision (which boxes need FIPS, which need FPGA tooling), not a defect to wait out. See reference.md trap 43 for the confirming test.
>
> An earlier version of this entry blamed a broken dependency in `ubuntu-fips-userspace` on a package noble does not ship. That was a misreading: an empty `apt-cache policy` version table means the name is **virtual**, not missing. Corrected here so nobody carries it into an assessment.
>
>
