...
| `/var/log` file perms (`file_permissions_var_log_stig`) | UBTU-24-700010 | strip setuid/exec/other bits off log files. Done **four** times: early in the role, again as its last task, on a daily `stig-log-perms.timer`, and once more immediately before every scan (`scap_scan` and `it-oscap` both start the unit). apt, dpkg and unattended-upgrades recreate their logs `0644` whenever they run — measured: one `apt-get install` after a clean sweep puts `/var/log/apt/history.log` and `eipp.log.xz` straight back to `0644`. `scap_scan` installs the SSG content package itself, so it was re-breaking the perms between the remediation and its own scan. That is why this rule kept failing on boxes whose remediation had run correctly minutes earlier |
| **Cron audit rules** (`audit_rules_etc_cron_d`, `audit_rules_var_spool_cron`) | UBTU-24-200270 | USG ships no watch on the cron spool, so this failed on every box. `/etc/audit/rules.d/72-cron.rules` adds `-w /etc/cron.d/ -p wa -k cronjobs` and `-w /var/spool/cron/ -p wa -k cronjobs`. The DISA check greps `auditctl -l` for those lines **verbatim, key included**, so the key is not a free choice. `/var/spool/cron` ships with the `cron` package, which the AI nodes do not have — it is created (`audit_cron_spool_create`) so the watch is loadable, because a watch on a missing path aborts the whole rule load. Loads on the post-build reboot (auditd is `-e 2`) |
...
| Audit-log offload (`auditd_offload_logs`) | UBTU-24-900240 | `/etc/cron.weekly/audit-offload` stages a compressed copy of the **rotated** audit logs under `/opt/ia/audit-offload`, pruned to the last 8. It can also collect **container logs and application audit trails** (`usg_audit_offload_containers`, `usg_audit_offload_extra`) into a separate `logs-*.tar.gz` — container output is not in `/var/log` or journald, so nothing else sees it — and push both to an SMB share (`usg_audit_offload_smb_*`, mounted on demand, credentials root-only and never in the repo). The local copy is kept even when the push succeeds. See [procedures.md §4.4b](procedures.md) |
...
| USB storage driver (`kernel_module_usb-storage_disabled`, UBTU-24-300039) | **EMI-classified only as of 2026-08-31.** development, ai and baseline now leave the module blacklisted and PASS this rule with no deviation — those boxes are being domain-joined, have no data-transfer role, and get no `dta` group or accounts. The remaining deviation applies to **the classified EMI laptop, where `usb_storage_enabled` is true** — the EMI laptop's `dta` workflow and the engineering workstations both move deliverables on approved removable media. Blacklisting the module removes the capability from everyone rather than restricting it to the authorised, and defeats the group-based mount restriction (no block device ever appears, so there is nothing to authorise). Compensating controls: USBGuard blocks unknown devices at enumeration and audits the decision; mounting is restricted to the `dta` group via polkit + udev. The AI nodes leave the module blacklisted and simply pass the rule. **`uas` is blacklisted alongside `usb-storage` on every profile where storage is off** — SSG's rule covers only `usb-storage`, but a USB3 drive speaking USB Attached SCSI binds `uas` instead, so the rule can pass while a modern USB SSD still attaches. Only the mass-storage class drivers are listed; dongles, serial/COM adapters and HID are unaffected and are governed by USBGuard | `usbguard` role, `usg_remediate` §5g2, `desktop_hardening` §4 |
...
> **FIPS mode is a POA&M on 24.04 (noble) as of 2026-09-08 — it cannot currently be enabled, and the cause is upstream.** `usg_enable_fips` is still `true` and `pro_attach` still runs `pro enable fips-updates`; it fails, the box is warned and the rest of the baseline applies. Confirmed on dev-16 and dev-17, both Pro-attached on a full Ubuntu Pro contract with `fips-updates` ENTITLED. Two independent blockers, established by querying apt while the repo was still configured (`pro` deletes it on failure — see reference.md trap 43):
>
> 1. **`ubuntu-fips-userspace` has a broken dependency.** Version `1.4.0~rc13+updates4` requires `libgnutls30 (= 3.8.3-1.1ubuntu3.6+Fips1.2)`. On noble that package does not exist under that name — the time_t transition renamed it, and **the FIPS suite itself ships the correct `libgnutls30t64` at exactly that version**. So the metapackage asks for a name its own repository does not publish. `apt-cache policy libgnutls30` → `Candidate: (none)`; `libgnutls30t64` → `Candidate: 3.8.3-1.1ubuntu3.6+Fips1.2` from `esm.ubuntu.com/fips-updates`. Nothing on the box can fix this. **Canonical case, with the `x-trace-id` values from `/var/log/ubuntu-advantage.log`.**
> 2. **`libgcrypt20` is pinned in place by i386 multiarch** — ours, on the development profile. It is `Multi-Arch: same` and the FIPS suite is amd64-only, so the amd64 copy cannot move to `1.12.0-2ubuntu0.1~Fips1~rc11` while an i386 copy sits at the archive version. Fixed by ordering (`pro_attach` before `fpga_tools`), which is necessary but **not sufficient** while (1) stands.
>
> **Wider context (checked 2026-09-08), and it matters more than the dependency bug.** Canonical's own [FIPS version tracker](https://people.canonical.com/~ubuntu-security/ubuntu-fips-tracker/) states that noble has **no FIPS versions of OpenSSL, libnettle, libgmp, libkcapi or strongSwan**. A FIPS kernel without a FIPS OpenSSL is not a FIPS posture, so the dependency defect above is not the only thing standing between this fleet and `is_fips_mode_enabled`. The tracker also lists noble at `libgcrypt20 1.10.3-...~Fips1~rc7` and `libgnutls30t64 3.8.3-1.1ubuntu3.4+Fips1.1`, while the suite served to dev-17 carries `1.12.0-2ubuntu0.1~Fips1~rc11` and `3.8.3-1.1ubuntu3.6+Fips1.2` -- an in-flight RC stream, including a major upstream bump of libgcrypt (1.10.3 -> 1.12.0). That churn is the most likely reason boxes built a few weeks earlier enabled FIPS and boxes built today cannot: a component was republished without the metapackage being rebuilt against it. Plausible and consistent with the evidence, not proven -- confirming it needs the `ubuntu-fips-userspace` version recorded on dev-13/14/15.
>
> The same failure SHAPE is long-standing upstream: [ubuntu-pro-client#2983](https://github.com/canonical/ubuntu-pro-client/issues/2983) reports `ubuntu-fips : Depends: libssl3 (= 3.0.2-0ubuntu1.14+Fips1) but 3.0.2-0ubuntu1.15 is to be installed` on jammy -- opened March 2024, still labelled "needs info", no resolution. Do not plan around a quick upstream fix. **If FIPS is an accreditation requirement rather than a nice-to-have, jammy is the release that has it; noble does not, today.**
>
> Note both FIPS versions here are release candidates (`~rc13`, `~rc11`) and `pro status --all` reports plain `fips` and `fips-preview` as `n/a` for noble.
>
> When Canonical fixes the metapackage: `pro enable fips-updates` swaps the running kernel and needs a reboot, so validate on a throwaway box if you run unusual crypto/dev tooling. Set `usg_enable_fips: false` to stop attempting it in the meantime.
>
