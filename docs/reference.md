...
**An explicit entry, unlike an inherited one, WIDENS a `0600` file** — `setfacl -m g:aiops:r` recalculates the mask and grants the read. The site owner has accepted that for everything under `/opt/stacks` and `/opt/docker`, every `.env` included (2026-09-25). **Private keys are the exception, and not by policy:** once a file carries an ACL entry its mode reads `0640` (the group bits are the mask), and OpenSSH refuses a key others can read — `WARNING: UNPROTECTED PRIVATE KEY FILE! Permissions 0640 ... are too open`, verified on noble's 9.6p1. magpie bind-mounts `./ssh` live, so granting its key would break git sync at the next scheduled run with no container restarted. `it-aiops` finds keys by content (a PEM/OpenSSH `PRIVATE KEY` header), holds them back, and strips an entry from any it finds.

Tested with a real unprivileged member: every `.env`, `.oikb.yaml`, `known_hosts` and the `.pub` files readable; both private keys denied and still accepted by `ssh-keygen -y`; a symlink to `/etc/shadow` denied (symlinks are never followed).
...
| `it-aiops` | AI review team (`aiops`): `on`/`off`/`refresh`/`add`/`remove` read access to everything in `/opt/stacks` + `/opt/docker` except private keys, by hand, no pull; bare = status |
