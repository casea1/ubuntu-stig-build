...
>
> Note both FIPS versions are release candidates (`~rc13`, `~rc11`) and `pro status --all` reports plain `fips` and `fips-preview` as `n/a` for noble. Treat noble FIPS as not yet shipping. The **jammy** boxes are unaffected.
>
