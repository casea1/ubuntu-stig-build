...
| `it-usb` | `usb-guard.sh` | USBGuard device allow-list: `enroll` (guided whitelist), `status`/`list`/`blocked`/`allow`/`block`/`policy`/`regenerate`. `allow`/`block` take **either** the leading device number from `it-usb list` **or** the `vendor:product` id (`0e90:0065`); an ambiguous pair lists the matches, and `--all` authorises every one — which is what a hub presenting USB2 and USB3 separately needs. |
